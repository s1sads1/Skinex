...
class RPartListAdapter(var arr: ArrayList<String>) : RecyclerView.Adapter<RPartListAdapter.RPartListViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RPartListViewHolder {

        context = parent.context
        val view = LayoutInflater.from(parent.context).inflate(R.layout.body_part_list, parent, false)

        return RPartListViewHolder(view)
    ...
    override fun onBindViewHolder(holder: RPartListViewHolder, position: Int) {
    ...
    inner class RPartListViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun whenLoad(position: Int) {
            ...
            var fb = if ((context as ReturningInfoActivity).findViewById<RadioButton>(R.id.bodyFront).isChecked) {
                "front"
            } else {
                "back"
            }
            ...
            (context as ReturningInfoActivity).findViewById<RecyclerView>(R.id.bodyPartDetailRecyclerView).layoutManager =
                GridLayoutManager(context, 3)
            (context as ReturningInfoActivity).findViewById<RecyclerView>(R.id.bodyPartDetailRecyclerView).adapter =
                RPartDetailAdapter(
                    fb,
                    arr[position],
                    context
